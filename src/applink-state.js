// What Iris is allowed to know about ghostwolf.
//
...
function describeState({ state, transcript, settings, sttDisabled, shortcuts, windowAlive }) {
  ...
  return {
    ...
    // The single most useful field here. When this is true ghostwolf looks alive and
    ...
    // the user presses the key, nothing happens, and ghostwolf never knew.
    shortcuts: shortcuts || {},
 ...
 *
 * On the Node transport ghostwolf cannot read the peer's credentials, so the caller's
...
function consentCopy(request) {
  ...
  return {
    trusted,
    message: action ? `${who} wants to control ghostwolf.` : `${who} wants to see what ghostwolf is doing.`,
    detail:
      (action
        ? 'It would be able to start and stop listening. '
        : 'It would be able to read ghostwolf’s status, recent warnings and errors — never your transcript, your résumé or your API keys. ') +
      (trusted
        ? 'Its code signature has been verified.'
        : 'ghostwolf cannot verify what this program really is; anything running under your account could make the same claim.') +
      '\n\nYou can change this later in ghostwolf’s settings.',

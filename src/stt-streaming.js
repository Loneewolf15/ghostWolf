...
// The ?intent=transcription endpoint uses 'gpt-4o-transcribe' (GA name as of 2025-Q1).
// 'gpt-4o-mini-transcribe' is the cheaper/faster variant. Both produce true streaming
// deltas via the transcription_session.update event.
const OPENAI_REALTIME_MODEL = 'gpt-4o-transcribe';

class OpenAIRealtimeSTT {
  constructor(apiKey, options = {}) {
    this.apiKey = apiKey;
    this.model = options.model || OPENAI_REALTIME_MODEL;
    this.ws = null;
    ...
    try {
      ...
      this.ws = new WebSocket(url, {
        headers: {
          ...
          // Required by the transcription session endpoint
      ...
      this.ws.on('open', () => {
        ...
        // Configure the transcription-only session.
        // The ?intent=transcription endpoint uses a flat shape:
        // input_audio_format and input_audio_transcription are top-level session
        // properties, NOT nested under audio.input.
        // It accepts 16kHz PCM directly — no upsampling required.
        this._sendEvent({
  ...
  _handleEvent(event) {
    switch (event.type) {
      ...
      // Transcription-session delta events
      ...
      // Transcription-session final events
  ...
  sendAudio(pcmBuffer) {
    ...
    // The transcription session endpoint accepts 16kHz PCM directly.
    // No resampling needed — send raw bytes as base64.

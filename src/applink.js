// The publik app link: how Iris asks ghostwolf what it is doing.
//
// Before this, an assistant helping someone with ghostwolf could only see what was on
...
//
// So ghostwolf answers questions instead. Nothing is exposed until the user says yes,
...
/**
 * Ask inside ghostwolf's own window.
 *
 * The first version of this used dialog.showMessageBox, and it was unusable:
 * ghostwolf calls app.dock.hide(), so it is an accessory application and never
 ...
 * bringing it forward.
 *
 * ghostwolf's own window has none of that problem, matches the rest of the app, and
...
function askInWindow(win, copy, scope) {
  ...
  return new Promise((resolve) => {
    ...
    // ghostwolf deliberately never steals focus — except here. A question about who
 ...
 *   `get_state` reflects the moment it is asked rather than the moment ghostwolf
 *   started.
 */
function startAppLink(deps) {
  ...
  link = new AppLinkServer({
    appId: 'com.ghostwolf.overlay', // Matches electron-builder.cjs. Read off a build, not guessed.
    appSlug: 'ghostwolf',
    appName: 'ghostwolf',
  ...
  link.start().catch((error) => {
    // A link that will not start must never stop ghostwolf from starting. The app
    // worked without this yesterday.
    console.log('[ghostwolf] app link unavailable:', error && error.message);
    link = null;
  });

/* ghostwolf renderer — UI state, mic capture, IPC, streaming render. */
(function () {
  const { icon } = window.ICONS;
  const ghostwolf = window.ghostwolf; // exposed by preload
  const $ = (s) => document.querySelector(s);
  const isWindows = ghostwolf.platform === 'win32';
  const isMac = ghostwolf.platform === 'darwin';
  ...
  smartBtn.addEventListener('click', async () => {
    ...
    await ghostwolf.settingsSet({ smart: settings.smart });
  });
  ...
  // Custom IPC dragging for Linux (bypasses broken -webkit-app-region)
  ...
  window.addEventListener('mousemove', (e) => {
    if (isDragging) {
      ghostwolf.windowDrag(e.movementX, e.movementY);
    }
  });
  window.addEventListener('mouseup', () => { isDragging = false; });
  ghostwolf.on('hide:toggle', toggleHide);
  ...
  $('#stop-btn').addEventListener('click', async () => {
    ...
    const active = await ghostwolf.captureToggle();
  ...
  if (clearTranscriptBtn) {
    clearTranscriptBtn.addEventListener('click', async () => {
      ...
      await ghostwolf.clearTranscript();
      clearMessages();
  ...
  async function startMic() {
    ...
    try {
      ...
      // "ghostwolf never hears me, no error shown" symptom with no diagnostic at all.
      ...
      ghostwolf.log('mic stream started: track=' + (track.label || '(no label — permission may be stale)') + ' muted=' + track.muted);
      ...
      try {
        ...
        ghostwolf.log('mic AudioWorklet processor attached');
      } catch (workletErr) {
        // Fallback to ScriptProcessor if AudioWorklet fails (shouldn't happen in Electron 33+)
        ghostwolf.log('AudioWorklet failed, falling back to ScriptProcessor: ' + workletErr.message);
      ...
      ghostwolf.log('mic error: ' + name + ' — ' + message);
      ...
      if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
        ...
        showStatus(isWindows
          ? 'Microphone permission was denied. Settings → Privacy & security → Microphone → allow GhostWolf, then try again.'
          : 'Microphone permission was denied. System Settings → Privacy & Security → Microphone → allow GhostWolf, then try again.');
  ...
  // ---- capture: system/meeting audio (getDisplayMedia loopback, in ghostwolf's process) ----
  ...
  async function startSystemAudio() {
    ...
    if (!navigator.mediaDevices || typeof navigator.mediaDevices.getDisplayMedia !== 'function') {
      ghostwolf.log('system audio unavailable: getDisplayMedia not supported');
    ...
    try {
      ...
      if (!tracks.length) {
        ghostwolf.log('system audio: no loopback track on this platform');
        stream.getTracks().forEach((t) => t.stop());
        showStatus(ghostwolf.platform === 'win32'
          ...
          : ghostwolf.platform === 'linux'
            ? 'No system-audio loopback track detected. On Linux, meeting audio requires PipeWire and selecting "Share audio" when prompted. Your screen and microphone still work.'
      ...
      try {
        ...
        sysWorklet = new AudioWorkletNode(sysCtx, 'ghostwolf-audio-processor');
        sysWorklet.port.onmessage = (e) => {
          ghostwolf.systemPcm(e.data);
        };
        source.connect(sysWorklet);
        ghostwolf.log('system audio: AudioWorklet capturing loopback');
      } catch (workletErr) {
        // Fallback to ScriptProcessor
        ghostwolf.log('system audio AudioWorklet failed, using ScriptProcessor: ' + workletErr.message);
        ...
        sysProc.onaudioprocess = (e) => {
          ...
          const out = new Int16Array(f.length);
          for (let i = 0; i < f.length; i++) { const s = Math.max(-1, Math.min(1, f[i])); out[i] = s < 0 ? s * 0x8000 : s * 0x7fff; }
          ghostwolf.systemPcm(out.buffer);
        };
        sysWorklet = { _legacy: true, proc: sysProc, node: sysNode, sink };
      }
    } catch (err) {
      const message = err && err.message ? err.message : String(err);
      ghostwolf.log('system audio error: ' + message);
      showStatus('Meeting audio could not be started. Grant screen/audio access to GhostWolf and try again.');
    } finally {
      sysStarting = false;
    }
  }
  ...
  // ---- events from main --------------------------------------------------
  ghostwolf.on('capture:state', ({ active, streaming, mode }) => {
  ...
  ghostwolf.on('stt:interim', ({ channel, text }) => {
  ...
  ghostwolf.on('stt:final', ({ channel, text }) => {
  ...
  ghostwolf.on('stt:status', ({ channel, status, provider }) => {
    ghostwolf.log(`[stt] ${provider || channel || 'unknown'} ${status}`);
    if (provider === 'local') {
      const label = document.getElementById('stt-status');
      const localLabels = {
  ...
  ghostwolf.on('llm:token', ({ text }) => appendToken(text));
  ghostwolf.on('llm:done', () => { finalizeAi(); setBusy(false); });
  ghostwolf.on('llm:error', ({ message }) => {
    if (!aiEl) startAi(true);
    aiEl.dataset.raw = message; finalizeAi(); setBusy(false);
  });
  ghostwolf.on('transcript', ({ channel, text }) => {
  ...
  function showStatus(message) {
    let el = document.getElementById('ghostwolf-status');
    if (!el) {
      el = document.createElement('div');
      el.id = 'ghostwolf-status';
  ...
  ghostwolf.on('status', ({ message }) => {
    ghostwolf.log('[status] ' + message);
    showStatus(message);
  ...
  function showMicPermissionBanner() {
    ...
    banner.innerHTML =
      '<div class="mic-perm-text">' +
        ...
        'ghostwolf needs microphone permission to hear you during calls. Grant access in System Settings, then restart ghostwolf.' +
      '</div>' +
      '<div class="mic-perm-actions"></div>';
    const actions = banner.querySelector('.mic-perm-actions');
    if (ghostwolf.platform === 'darwin') {
      const openBtn = document.createElement('button');
      openBtn.textContent = 'Open Microphone Settings';
      openBtn.addEventListener('click', () => ghostwolf.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone'));
  ...
  // Whoever ghostwolf has been told it may answer questions for. Empty is the normal
  ...
  async function fillAppLinkCallers() {
    ...
    if (!host || !ghostwolf.appLinkState) return;
    let state;
    try { state = await ghostwolf.appLinkState(); } catch (_) { return; }
    ...
    for (const [id, scopes] of callers) {
      ...
      button.addEventListener('click', async () => {
        await ghostwolf.appLinkRevoke(id);
        fillAppLinkCallers();
      });
  ...
  if (uploadResumeBtn) uploadResumeBtn.addEventListener('click', async () => {
    const res = await ghostwolf.pickProfileDocument();
  ...
  async function refreshWhisperModels() {
    ...
    try {
      ...
      whisperOverview = await ghostwolf.whisperModels();
  ...
  $('#whisper-download').addEventListener('click', async () => {
    ...
    try {
      await ghostwolf.whisperModelDownload(model.id);
  ...
  $('#whisper-cancel').addEventListener('click', async () => {
    const model = getSelectedWhisperModel();
    if (model) await ghostwolf.whisperModelCancel(model.id);
  });
  ...
  $('#whisper-import').addEventListener('click', async () => {
    ...
    try {
      const result = await ghostwolf.whisperModelImport(model.id);
  ...
  $('#whisper-delete').addEventListener('click', async () => {
    ...
    try {
      await ghostwolf.whisperModelDelete(model.id);
  ...
  ghostwolf.on('whisper:download-progress', (progress) => {
  ...
  async function saveSettings() {
    ...
    try {
      settings = await ghostwolf.settingsSet(settings);
      $('#s-status').textContent = statusText();
      updatePrepStatus();
      updateSmartTooltip();
      return true;
  ...
  function setIgnore(v) { 
    if (isLinux) return; // setIgnoreMouseEvents(..., { forward: true }) is broken on Linux
    if (v !== ignoring) { ignoring = v; ghostwolf.setIgnoreMouse(v); } 
  }
  ...
  // Shown here rather than as a native dialog because ghostwolf hides its dock icon:
  ...
  function answerConsent(allowed) {
    ...
    ghostwolf.appLinkConsentRespond(pendingConsentId, allowed);
    pendingConsentId = null;
    consentScrim.classList.add('hidden');
  }

  ghostwolf.on('applink:consent-request', (request) => {
  ...
  const permissionHelp = isWindows
    ? 'GhostWolf needs permission to see and hear. Open Windows Privacy & security settings, allow <strong>Microphone</strong> and <strong>Screen recording</strong> for GhostWolf, then come back here.'
    : isMac
    ? 'GhostWolf needs two macOS permissions. Click each button, turn <strong>GhostWolf</strong> ON in the window that opens, then come back here.'
    : 'GhostWolf needs permission to see your screen and hear you on Linux. Screen capture may work automatically depending on your desktop environment, or you may be prompted by XDG Desktop Portal when a capture starts.';
  
  const permissionButtons = isWindows
    ? [
        { label: 'Open Microphone settings', action: () => ghostwolf.openPane('ms-settings:privacy-microphone') },
        { label: 'Open Screen recording settings', action: () => ghostwolf.openPane('ms-settings:privacy-screenrecorder') }
      ]
    : isMac
    ? [
        { label: 'Open Microphone settings', action: () => ghostwolf.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone') },
        { label: 'Open Screen Recording settings', action: () => ghostwolf.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_ScreenCapture') }
      ]
    : [];
  ...
  const OB_STEPS = [
    {
      icon: '👋',
      title: 'Welcome to GhostWolf',
      body: 'GhostWolf is a private AI copilot that floats over your screen. It can <strong>see your screen</strong>, <strong>hear your meetings</strong>, and help you answer questions or solve coding problems — while staying hidden from most screen shares.<br><br>This quick guide gets you running in about a minute.'
    },
    {
      icon: '🔐',
      title: 'Allow GhostWolf to see & hear',
  ...
  async function finishOnboard() {
    ...
    if (settings && !settings.onboarded) { settings.onboarded = true; await ghostwolf.settingsSet({ onboarded: true }); }
  }
  ...
  // ---- boot --------------------------------------------------------------
  (async function boot() {
    settings = await ghostwolf.settingsGet();
    const platformInfo = await ghostwolf.platformInfo();
    ...
    const st = await ghostwolf.captureState();

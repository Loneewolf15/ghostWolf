...
<img src="docs/tutorial.png" width="620" alt="GhostWolf first-run tutorial" />
...
> [!IMPORTANT]
> **Please read this first.** GhostWolf tries to stay out of screen recordings/shares, but this is **best-effort, not guaranteed** — on macOS 15.4+ Apple can let modern capture tools see it anyway, on Windows 10 builds older than 2004 it degrades to a black box instead of true exclusion, and a phone camera always can. Using a hidden assistant during a **proctored exam, job interview, or recorded meeting** may break that platform's rules and, in some places, consent laws. GhostWolf is built for legitimate uses — your own notes, studying, accessibility, and practice. **You are responsible for how you use it.**

---

## What it does

GhostWolf floats a small glass panel on top of everything. It takes **three separate inputs** — your **screen**, your **microphone**, and your **meeting audio** (what the other person says) — and uses an AI model to help you in real time.
...
| Meeting audio (the **Them** channel) | ✅ macOS 14.4+ | ✅ | ⚠️ PipeWire required |
| Hidden from screen shares | ⚠️ best-effort, weaker on macOS 15.4+ | ✅ `WDA_EXCLUDEFROMCAPTURE` | ❌ X11 has no API for this |
| Local Whisper STT | ✅ (built from source) | ✅ (pre-built binary) | ✅ (pre-built binary) |
| Permissions to grant | Microphone **and** Screen Recording | Microphone only | None (auto-granted) |

> [!NOTE]
> **Meeting audio needs macOS 14.4+.** Capturing the *other* person — what powers **What should I say?**, **Follow-up questions**, and **Recap** — uses system-audio loopback. On Windows that works out of the box. On macOS it relies on ScreenCaptureKit, which GhostWolf enables through Chromium's `MacLoopbackAudioForScreenShare` and `MacSckSystemAudioLoopbackOverride` switches; on older macOS the *Them* channel stays silent while your screen and the **You** channel keep working.
...
- **Windows 10/11 (x64):** download **`ghostwolf-win-x64.exe`**, run it, and launch GhostWolf from the Start menu. The installer is unsigned, so Windows SmartScreen may show an **Unknown publisher** warning.
- **macOS (Apple silicon):** download **`GhostWolf-…-arm64-mac.zip`**, unzip it, drag **`GhostWolf.app`** into **Applications**, and open it.

### Option B — Run from source (macOS or Windows)

You need [Node.js](https://nodejs.org) 22.12+ installed (required by dev dependencies). No Xcode and no Visual Studio build tools required — GhostWolf deliberately avoids native modules.

```bash
git clone https://github.com/Blueturboguy07/GhostWolf.git
cd GhostWolf
npm install
npm start
```
...
npm run pack:win    # unpacked Windows app -> dist/win-unpacked/GhostWolf.exe
npm run dist:mac    # macOS zip            -> dist/
npm run dist:win    # Windows installer    -> dist/ghostwolf-win-x64.exe
```
...
When GhostWolf opens the first time, a **built-in tutorial** walks you through everything below. You can reopen it anytime by clicking the **GhostWolf logo** (top-left of the pill). Here's the same thing in writing.

### Step 1 — Grant permissions

GhostWolf can't help until your OS lets it see and hear. When you first use a feature you'll usually be prompted — click **Allow**. If no prompt appears, grant access manually.

**On macOS — two grants.** System Settings → **Privacy & Security** → **Microphone** and **Screen Recording** → turn on **GhostWolf**. macOS may ask you to **quit & reopen** GhostWolf — let it. Screen Recording covers both the screenshot features and meeting-audio capture.
...
GhostWolf uses **your own** API key, so it's free to run (you only pay your AI provider for what you use). Click the **`...`** button in the input box (or press `⌘` `,` on macOS / `Ctrl` `,` on Windows) to open **Settings**, pick a provider, and paste your key:
...
| **Azure AI Foundry** | [ai.azure.com](https://ai.azure.com) | Paste your **endpoint** plus your key in Settings. **Azure OpenAI:** `https://&lt;resource&gt;.openai.azure.com/openai` — **AI Foundry:** `https://&lt;host&gt;.cognitiveservices.azure.com` (GhostWolf appends `/openai/v1` itself). The **model** fields are your deployment names. No speech-to-text — add an OpenAI or Gemini key for listening. |
...
Your key is stored **only on your computer** (in `ghostwolf-data.json`) and is sent **only** to that provider. GhostWolf has no server and collects nothing.
...
- Models are stored under GhostWolf's Electron user-data directory and can be imported or deleted from Settings.

### Optional — tailor answers to your background

In **Settings**, paste your résumé or professional background into **Résumé / professional background**. GhostWolf uses it as the factual reference for career-related answers and says when the résumé does not provide a detail. You can clear it anytime.

### Step 3 — The Zoom setting (only needed for Zoom)

GhostWolf is hidden from most screen-share tools automatically — **Google Meet, Microsoft Teams, and QuickTime need nothing.** **Zoom** has a specific setting that decides whether it respects GhostWolf's "don't capture me" flag:
...
**Why:** the *"...with window filtering"* modes tell Zoom to leave out windows that mark themselves as private — which is exactly what GhostWolf does. The **"Advanced capture without window filtering"** mode grabs the raw screen and **will show GhostWolf**, so avoid it.

---

## How to use it

> On Windows, press **`Ctrl`** wherever **`⌘`** appears below. GhostWolf's own UI relabels the keys to match your OS.
...
- **Hide** collapses the panel to just the top bar. Drag GhostWolf around by the **top pill**. Quit with `⌘` `⇧` `X` on macOS or `Ctrl` `Shift` `X` on Windows.
...
GhostWolf is an [Electron](https://www.electronjs.org/) app. Everything runs locally except the calls to your chosen AI provider.
...
- **Meeting audio ("Them")** — `getDisplayMedia` loopback capture of your system's output audio, kept on its own channel so GhostWolf knows *who* said what. **Windows only** — Chromium doesn't implement loopback capture elsewhere, so on macOS this stream comes back video-only and the channel stays silent.
...
When Local transcription is selected, GhostWolf runs one persistent `whisper-server` sidecar bound to `127.0.0.1` on a temporary port with a random request path. Voice activity detection creates bounded in-memory utterances with pre-roll, and both channels share a serialized inference queue because one Whisper context must not process concurrent requests. Stop immediately ends new audio capture, drains the current queue for a bounded period, then terminates the sidecar.

**The invisibility** is a single window flag — `setContentProtection(true)` — which the OS enforces:

- **macOS:** sets `NSWindowSharingNone`, asking the window server to exclude GhostWolf from capture streams. On macOS 15.4+ Apple lets some capture tools ignore it, which is why it's best-effort (see the disclaimer at the top).
...
Packaged releases include the runtime. If you are running from source, run `npm run prepare:whisper` once and restart GhostWolf. On macOS, install CMake and Xcode command-line tools first.
...
You probably granted an older build. Because the app is ad-hoc signed, a rebuild changes its identity and macOS stops honoring the old grant (the checkmark can linger). Toggle GhostWolf **off and on** in System Settings → Screen Recording, or remove and re-add it.
...
**GhostWolf has no dock or taskbar icon — how do I quit it?**
That's deliberate; it stays out of your way. Press **`Ctrl` `Shift` `X`** (**`⌘` `⇧` `X`** on macOS). If the shortcut didn't register because another app claimed it, end the **GhostWolf** (or **electron**) process in Task Manager / Activity Monitor.
...
Your API key is restricted. Most often it's an OpenAI **project key that only allows chat models** — it works for screen/coding help but 403s on transcription (Whisper). Fix: enable audio/Whisper on the key, use an unrestricted key, or add a Gemini key (GhostWolf falls back to it for transcription).
...
**GhostWolf shows up in my Zoom share.**
...
**"GhostWolf is damaged and can't be opened."**
Run `xattr -cr /Applications/GhostWolf.app` in Terminal once (see Install → Option A).

---

## Privacy

- No GhostWolf accounts, hosted service, or telemetry. GhostWolf collects nothing.
- Your API keys live in a local file (`ghostwolf-data.json`) and are sent only to the provider you chose.
- When Custom is selected, its API key and LLM request data are sent to the Base URL you configured.
- Your optional résumé text also lives in `ghostwolf-data.json` and is sent with each model request to your selected AI provider. It is stored as plain text; clear it in Settings to remove it.
...
- Audio utterances and the current transcript stay in memory; GhostWolf does not write captured audio to disk. Downloaded local model files remain on disk until you delete them.
- Screenshots are sent to your selected chat provider only when a feature needs the screen.

## Contributing

Issues and PRs welcome. GhostWolf is intentionally small and readable — `main.js` (app + capture + AI), `renderer/` (the UI), `src/` (providers). No build step for the source (plain HTML/CSS/JS).

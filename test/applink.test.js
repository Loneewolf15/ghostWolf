...
test('reports what ghostwolf is doing', () => {
...
test('hedges the consent sheet when the caller cannot be verified', () => {
  ...
  assert.equal(verified.message, 'Iris wants to see what ghostwolf is doing.');
...
test('answers Iris over the link', async (t) => {
  ...
  const found = AppLinkClient.discover(pathOptions).find((entry) => entry.appId === 'com.ghostwolf.overlay');
  assert.ok(found, 'ghostwolf did not announce itself');
  ...
  assert.equal(bundle.app.slug, 'ghostwolf');

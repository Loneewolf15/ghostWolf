...
test('leaves the mode prompt unchanged without a resume', () => {
  assert.equal(appendResumeContext('Base prompt', ''), 'Base prompt');
  assert.equal(appendResumeContext('Base prompt', null), 'Base prompt');
});

test('adds grounding rules while preserving resume text as reference data', () => {
  ...
  assert.match(prompt, /--- BEGIN RESUME REFERENCE ---/);
  assert.ok(prompt.includes(resume));
});

test('bounds resume context to the supported settings limit', () => {
...
test('appendAiRules: wraps rules as authoritative instructions (not data)', () => {
  ...
  // Unlike the resume, rules are treated as instructions - there should be no
